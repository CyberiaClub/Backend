...
import pe.edu.pucp.cyberiastore.usuario.daoImpl.Tipo_Operacion_Usuario;
...
public class UsuarioDAOImpl extends DAOImpl implements UsuarioDAO {
    ...
    private Tipo_Operacion_Usuario tipoModif;
    ...
    public Integer modificar(Usuario usuario, Boolean usarTransaccion, Connection conexion) {
        this.tipoModif = Tipo_Operacion_Usuario.MODIFICAR_TODO;
        this.usarTransaccion = usarTransaccion;
        this.conexion = conexion;
    ...
    public Integer modificarRol(Usuario usuario, Boolean usarTransaccion, Connection conexion) {
        this.tipoModif = Tipo_Operacion_Usuario.MODIFICAR_ROL;
        this.usarTransaccion = usarTransaccion;
        this.conexion = conexion;
        return this.modificar(usuario);
    }
    ...
    protected String obtenerListaDeValoresYAtributosParaModificacion() {
        if (this.tipoModif == Tipo_Operacion_Usuario.MODIFICAR_TODO) {
            return "DOCUMENTO=?, TELEFONO=?, NOMBRE=?,APELLIDO_PATERNO=?, APELLIDO_MATERNO =?,SEXO=?, FECHA_NACIMIENTO=?, CORREO=?, DIRECCION=?, ACTIVO=?, CONTRASEÑA=?, NACIONALIDAD=?, TIPO_DOCUMENTO=?,ROL=?";
        } else if (this.tipoModif == Tipo_Operacion_Usuario.MODIFICAR_ROL) {
            return "ROL=?";
        }
        return "";
    }
    ...
    protected void incluirValorDeParametrosParaModificacion() throws SQLException {
        if (this.tipoModif == Tipo_Operacion_Usuario.MODIFICAR_TODO) {
            ...
            this.incluirParametroString(1, this.usuario.getRol().toString());
        }
    }
