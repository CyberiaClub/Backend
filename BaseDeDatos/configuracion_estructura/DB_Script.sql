...
CREATE TABLE IF NOT EXISTS `CyberiaStore`.`PRODUCTO_X_SEDE` (
  ...
  INDEX `fk_PRODUCTO_X_SEDE_SEDE1_idx` (`ID_SEDE` ASC) VISIBLE,
  CONSTRAINT `fk_PRODUCTO_has_SEDE_PRODUCTO1`
    FOREIGN KEY (`ID_PRODUCTO`)
    REFERENCES `CyberiaStore`.`PRODUCTO` (`ID_PRODUCTO`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_PRODUCTO_X_SEDE_SEDE1`
    FOREIGN KEY (`ID_SEDE`)
...
CREATE TABLE IF NOT EXISTS `CyberiaStore`.`PRODUCTO_X_OFERTA` (
  ...
  INDEX `fk_PRODUCTO_has_OFERTA_OFERTA1_idx` (`ID_OFERTA` ASC) VISIBLE,
...
CREATE TABLE IF NOT EXISTS `CyberiaStore`.`PERSONA` (
  ...
  INDEX `fk_PERSONA_TIPO_TRABAJADOR1_idx` (`ID_TIPO_PERSONA` ASC) VISIBLE,
  INDEX `fk_PERSONA_SEDE1_idx` (`ID_SEDE` ASC) VISIBLE,
  CONSTRAINT `fk_PERSONA_TIPO_TRABAJADOR1`
    FOREIGN KEY (`ID_TIPO_PERSONA`)
    REFERENCES `CyberiaStore`.`TIPO_PERSONA` (`ID_TIPO_PERSONA`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
...
CREATE TABLE IF NOT EXISTS `CyberiaStore`.`TIPO_PERSONA_X_PAGINA` (
  ...
  INDEX `fk_TIPO_PERSONA_has_PAGINA_PAGINA1_idx` (`ID_PAGINA` ASC) VISIBLE,
  INDEX `fk_TIPO_PERSONA_has_PAGINA_TIPO_PERSONA1_idx` (`ID_TIPO_PERSONA` ASC) VISIBLE,
  CONSTRAINT `fk_TIPO_PERSONA_has_PAGINA_TIPO_PERSONA1`
    FOREIGN KEY (`ID_TIPO_PERSONA`)
    REFERENCES `CyberiaStore`.`TIPO_PERSONA` (`ID_TIPO_PERSONA`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_TIPO_PERSONA_has_PAGINA_PAGINA1`
    FOREIGN KEY (`ID_PAGINA`)
    REFERENCES `CyberiaStore`.`PAGINA` (`ID_PAGINA`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
