...
CREATE TABLE IF NOT EXISTS `CyberiaStore`.`COMPROBANTE_PAGO` (
  ...
  INDEX `fk_COMPROBANTE_DE_PAGO_TIPO_TRANSACCION1_idx` (`ID_TIPO_COMPROBANTE` ASC) VISIBLE,
  INDEX `fk_COMPROBANTE_DE_PAGO_USUARIO1_idx` (`ID_PERSONA` ASC) VISIBLE,
  CONSTRAINT `fk_COMPROBANTE_DE_PAGO_TIPO_TRANSACCION1`
    FOREIGN KEY (`ID_TIPO_COMPROBANTE`)
    REFERENCES `CyberiaStore`.`TIPO_COMPROBANTE` (`ID_TIPO_COMPROBANTE`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_COMPROBANTE_DE_PAGO_USUARIO1`
    FOREIGN KEY (`ID_PERSONA`)
    REFERENCES `CyberiaStore`.`PERSONA` (`ID_PERSONA`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
...
CREATE TABLE IF NOT EXISTS `CyberiaStore`.`COMPROBANTE_PAGO_X_PRODUCTO` (
  ...
  INDEX `fk_COMPROBANTE_DE_PAGO_has_PRODUCTO_PRODUCTO1_idx` (`ID_PRODUCTO` ASC) VISIBLE,
  INDEX `fk_COMPROBANTE_DE_PAGO_has_PRODUCTO_COMPROBANTE_DE_PAGO1_idx` (`ID_COMPROBANTE_PAGO` ASC) VISIBLE,
  CONSTRAINT `fk_COMPROBANTE_DE_PAGO_has_PRODUCTO_COMPROBANTE_DE_PAGO1`
    FOREIGN KEY (`ID_COMPROBANTE_PAGO`)
    REFERENCES `CyberiaStore`.`COMPROBANTE_PAGO` (`ID_COMPROBANTE_DE_PAGO`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_COMPROBANTE_DE_PAGO_has_PRODUCTO_PRODUCTO1`
